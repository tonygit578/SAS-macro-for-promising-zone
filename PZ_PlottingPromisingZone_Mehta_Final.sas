...
** It is a SAS implementation to the optimal promising zone design proposed by Mehta et al (2011) -------------
...
%macro Data4PZGraph_Mehta(n1 =, n2 =, cpmax = 0.9, nmax =, alpha = 0.05, randR = 1, dsout = );
	data KeyQuanti;
	...
	/* this is to assign the expected values to macro variables, the first one
		corresponds to CPmin, and the 2nd corresponds to CPmax */
	proc sql noprint;
	   select condiPowerAtIA into: cp separated by ','
	   from FindRecCPmin; 
	   select n2_real_inc into: n2real_inc separated by ','
	   from FindRecCPmin; 
	   select z1 into: z1V separated by ','
	   from FindRecCPmin; quit; 

	%global cpmin z1_L z1_U n2act_inc;

	%let cpmin = %sysfunc(strip(%sysfunc(scan(%bquote(&cp),1, ','))));
	%let z1_L = %sysfunc(strip(%sysfunc(scan(%bquote(&z1V),1, ','))));
	%let z1_U = %sysfunc(strip(%sysfunc(scan(%bquote(&z1V),2, ','))));
	%let n2act_inc = %sysfunc(strip(%sysfunc(scan(%bquote(&n2real_inc),1, ','))));

	%put &cpmin &z1_L &z1_U &n2act_inc;
	...
	data KeyQuanti;
		...
		z1_L	  = round(&z1_L, 0.0001);
		z1_C	  = round((sqrt(&n2act_inc/n1) + sqrt(n1/(n2 - n1)))**(-1)*(z_alpha1s * sqrt(n2/(n2 - n1)) + z_cpmax), 0.0001);
		z1_U	  = round(&z1_U, 0.0001);
	...
	data PrepData4PZ1;
		...
		if z1 <= z1_L then do;
			...
			V4CP_min = z1 * sqrt((nmax - n1)/n1) - (z_alpha1s * sqrt(n2) - sqrt(n1)*z1)/sqrt(n2 - n1);
		...
		if z1_L < z1 <= z1_C then do;
			...
			V4CP_inc = z1 * sqrt((nmax - n1)/n1) - (z_alpha1s * sqrt(n2) - sqrt(n1)*z1)/sqrt(n2 - n1);
	...
	run;
%mend;
%Data4PZGraph_Mehta(n1 = 140, n2 = 280, cpmax = 0.9, nmax = 420, alpha = 0.05, randR = 1, dsout = PrepData4PZ3);


/* printing the relevant information for the key quantities */
proc print data = KeyQuanti noobs;
	var n1 n2 nmax cpmin z1_L z1_C z1_U HR_L HR_C HR_U;
run;
...
data label4Z;
	...
	function = "text"; x1 = &LowerZ; y1 = 0; label = "z^{sub '1'}^{sup 'L'}"; drawspace = "datavalue"; output;
	function = "text"; x1 = &CentrZ; y1 = 0; label = "z^{sub '1'}^{sup 'C'}"; drawspace = "datavalue"; output;
	function = "text"; x1 = &UpperZ; y1 = 0; label = "z^{sub '1'}^{sup 'U'}"; drawspace = "datavalue"; output;
run;
...
ods graphics / imagename = "PZ_G4Z" imagefmt = png; 
...
**-------------------------------------------------------------------------------------------------------------;
ods listing gpath = 'G:\Papers for Publication\PharmSUG\Promising zone\Software\Output';
ods graphics / imagename = "PZ_G4HR" imagefmt = png; 
